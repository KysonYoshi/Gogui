...
{
    public Field(DrawProperty draw)
    {
        m_draw = draw;
    }
    
    public DrawProperty m_draw;
    ...
    {
        x -= (size * m_draw.getWidthRatio() - size) / 2;
        y -= (size * m_draw.getHeightRatio() - size) / 2;
        if (! graphics.hitClip(x, y, size, size))
            return;
        m_graphics = graphics.create(x, y, (int)(size * m_draw.getWidthRatio()), (int)(size * m_draw.getHeightRatio()));
    ...
    private void drawCircle(Color color)
    {
        ...
        int w = m_size * 2 / 10;
        int x = (int)((m_size * m_draw.getWidthRatio() - w) / 2);
        int y = (int)((m_size * m_draw.getHeightRatio() - w) / 2);
        
        m_graphics.fillOval(x, y, w, w);
    }
    ...
    private void drawFieldColor()
    {
        ...
        m_draw.fillBackGround(m_graphics, (int)(m_size * m_draw.getWidthRatio()), (int)(m_size * m_draw.getHeightRatio()), m_fieldColor);
        m_graphics.setPaintMode();
    }
                           ...
                           Image boardImage, int boardWidth)
    {
        ...
        int x = Math.max(((int)(m_size * m_draw.getWidthRatio()) - width) / 2, 0);
        int y = (ascent + (int)(m_size * m_draw.getHeightRatio())) / 2;
        if (m_ghostStone == null)
        {
            if (m_color == BLACK)
    ...
    private void drawMarks()
    {
        ...
        int d = m_size / 4;
        int x = (int)(m_size * m_draw.getWidthRatio() / 2) - d;
        int y = (int)(m_size * m_draw.getHeightRatio() / 2) - d;
        ...
        if (m_mark)
        {
            m_graphics.drawLine(x, y, x + width, y + width);
            m_graphics.drawLine(x, y + width, x + width, y);
        }
        if (m_markCircle)
            m_graphics.drawOval(x, y, width - 1, width - 1);
        if (m_markSquare)
            m_graphics.drawRect(x, y, width - 1, width - 1);
        if (m_markTriangle)
        {
            int height = (int)(0.866 * width);
            int top = (int)(0.866 * (width - height) / 2);
            int bottom = top + height - 1;
            m_graphics.drawLine(x, y + bottom, x + width / 2, y + top);
            m_graphics.drawLine(x + width / 2, y + top,
                                x + width, y + bottom);
            m_graphics.drawLine(x + width, y + bottom, x, y + bottom);
        }
        if (oldStroke != null)
            m_graphics2D.setStroke(oldStroke);
        m_graphics.setPaintMode();
                           ...
                           Color colorBright, boolean isGhostStone)
    {
        ...
        m_graphics.fillOval((int)(margin + ((m_size * m_draw.getWidthRatio() - m_size) / 2)), (int)(margin + ((m_size * m_draw.getHeightRatio() - m_size) / 2)),

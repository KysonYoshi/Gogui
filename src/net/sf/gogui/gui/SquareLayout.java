    ...
    implements LayoutManager
{
    ...
    public void layoutContainer(Container parent)
    {
        ...
        int len = size.width < size.height * m_ratio[0] / m_ratio[1] ? size.width *m_ratio[1] / m_ratio[0]: size.height;
        int x = (size.width - len * m_ratio[0] / m_ratio[1]) / 2;
        int y = (size.height - len) / 2;
        parent.getComponent(0).setBounds(x + insets.left, y + insets.top,
                                         len * m_ratio[0] / m_ratio[1], len);
    }

...
import net.sf.gogui.boardpainter.DrawBoard;
import net.sf.gogui.boardpainter.FieldFactory;
import net.sf.gogui.boardpainter.GoFieldFactory;
...
import net.sf.gogui.util.ConditionPattern;
...
import net.sf.gogui.rule.RuleUtil;
            ...
            ContextMenu.Listener, LiveGfx.Listener
{
        ...
        throws GtpError, ErrorMessage
    {
        ...
        m_guiBoard = new GuiBoard(boardSize, new GoFieldFactory());
        ...
        m_showRZone = m_prefs.getBoolean("show-rzone", true);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                // set default game
                m_actions.m_actionGo.actionPerformed(null);
            } });
    }
    ...
    public void actionBoardSize(int size)
    {
        ...
        actionNewGame(size);
        if (m_rule != null)
            m_prefs.putInt(m_rule.gameName() + "-boardsize", size);
        else
    ...
    public void actionFindFromTextField()
    {
        if (! checkStateChangePossible())
            return;
        // Get initial value from highlighted area
        String initial_value = m_comment.getSelectedText();
        if (! StringUtil.isEmpty(initial_value))
            m_patternTextField.setText(initial_value);
        String condition_regex = m_patternTextField.getText();
        if (StringUtil.isEmpty(condition_regex))
            return;
        if (condition_regex.equals(m_currentSearchPattern)) {
            actionFindNext();
            return;
        }
        try
        {
            m_cpattern = new ConditionPattern(condition_regex);
            m_currentSearchPattern = condition_regex;
        }
        catch (Exception e/* PatternSyntaxException e */)
        {
        ...
        if (NodeUtil.commentContains(getCurrentNode(), m_cpattern))
            m_comment.markAll(m_cpattern);
        else
            actionFindNext();
    }
    ...
    public void actionFind()
    {
        if (! checkStateChangePossible())
            return;
        ConditionPattern cpattern = FindDialog.run(this, m_comment.getSelectedText(),
                m_messageDialogs);
        if (cpattern == null)
            return;
        m_cpattern = cpattern;
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                try
                {
                    ...
                    ConstNode node =
                            NodeUtil.findInComments(currentNode, m_cpattern);
    ...
    public void actionForwardMaxNode(int n)
    {
        if (! checkStateChangePossible())
            return;
        boolean protectGui = (m_gtp != null && n > 1);
        actionGotoNode(NodeUtil.forwardMaxNode(getCurrentNode(), n), protectGui);
    }
    ...
    private FieldFactory m_fieldFactory = new GoFieldFactory();
    
    public void actionSetGame(Rule rule, DrawBoard drawBoard, FieldFactory fieldFactory, int defaultBoardSize)
    {
        if (! checkStateChangePossible())
            return;
        if (! checkSaveGame())
        ...
        m_guiBoard.setFieldFactory(fieldFactory);
        m_guiBoard.m_painter.m_drawBoard = drawBoard;
        int boardSize = m_prefs.getInt(m_rule.gameName() + "-boardsize",  defaultBoardSize);
        actionBoardSize(boardSize);
        getBoard().setRule(m_rule);
    }
    ...
    public void actionToggleShowRZone()
    {
        m_showRZone = ! m_showRZone;
        m_prefs.putBoolean("show-rzone", m_showRZone);
        updateFromGoBoard();
        updateViews(false);
    }
    ...
    public void fieldClicked(GoPoint p, boolean modifiedSelect)
    {
        ...
        else
        {
            ...
            if (m_rule != null)
            {
                if (m_rule.isEnd()){
                    m_game.haltClock();
                    showGameRuleFinished();
                    return;
                }
                
                if (!m_rule.isLegalMove(RuleUtil.goPointToRulePoint(p, getBoard().getColor(p))) && 
                        !showQuestion(m_rule.getIllegalMoveMainMessage(), m_rule.getIllegalMoveoptionalMessage(), m_rule.getIllegalMoveDestructiveOption(),  false))
                    return;                
            }
            else
            {
...
//    }
    ...
    public ConditionPattern getPattern()
    {
        return m_cpattern;
    }
    ...
    private ConditionPattern m_cpattern;
    ...
    private JTextField m_patternTextField = new JTextField();
    
    private String m_currentSearchPattern = "";
    ...
    private void drawRZone()
    {
        final Color[] COLORS = new Color[] {
            new Color(0xff, 0x00, 0x00), // R_COLOR
            new Color(0x00, 0xff, 0x40), // G_COLOR
            new Color(0x0d, 0xc8, 0xf0)  // B_COLOR
        };
        int board_size = getBoardSize();
        if (m_showRZone == false)
        {
            for (int row = 0; row < board_size; row++)
                for (int col = 0; col < board_size; col++)
                {
                    GoPoint point = GoPoint.get(col, row);
                    m_guiBoard.setFieldBackground(point, null);
                }
            return;
        }
        ConstNode curr_node = getCurrentNode();
        ConstSgfProperties sgf_prop = curr_node.getSgfPropertiesConst();
        if (sgf_prop == null)
            return;
        for (int k = COLORS.length - 1; k >= 0; k--)
        {
            try
            {
                long color_code = Long.parseLong(sgf_prop.getValue("RZ", k), 16);
                Color color = COLORS[k];
                for (int row = 0; row < board_size; row++)
                    for (int col = 0; col < board_size; col++)
                    {
                        final int bit_idx = row * board_size + col;
                        GoPoint point = GoPoint.get(col, row);
                        if (((color_code >> bit_idx) & 1) != 0)
                            m_guiBoard.setFieldBackground(point, color);
                    }
            }
            catch (Exception e)
            {
    ...
    private void showGameRuleFinished()
    {
        if (m_resigned)
            return;
        m_messageDialogs.showInfo(this, m_rule.getEndMoveMainMessage(),
                m_rule.getEndMoveoptionalMessage(), false);
    }
